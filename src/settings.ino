...
void RtcSettingsLoad() {
	...
	RtcSettings.NoiseFloor = 1;
	RtcSettings.WatchdogThreshold = 1; //2;
        RtcSettings.SpikeRejection = 1; //2;
...
void RtcInit() {
    strlcpy(ntp_servers_local[0], NTP_SERVER1, sizeof(ntp_servers_local[0]));
    strlcpy(ntp_servers_local[1], NTP_SERVER2, sizeof(ntp_servers_local[1]));
    strlcpy(ntp_servers_local[2], NTP_SERVER3, sizeof(ntp_servers_local[2]));
    sntp_setservername(0, ntp_servers_local[0]);
    sntp_setservername(1, ntp_servers_local[1]);
    sntp_setservername(2, ntp_servers_local[3]);
//    for (int i=0; i < sizeof(ntp_servers_local)/sizeof(ntp_servers_local[0]); i++) {
//	ping_ntp_sever(ntp_servers_local[i]);	
//    }
    sntp_stop();
